...
int Level::clear_line(int p_lvl) {
    ...
    for (int y = std::max(p_lvl - 1, 1); y < std::min(p_lvl + 3, (int)LVL_SIZE_Y); y++) {
        bool cleared = true;
        for (int x = 1; x < LVL_SIZE_X; x++) {
            // If all cells on Y level are not empty, then the level is cleared.
            if (get_cell({ x, y }).color == Renderer::DEFAULT) {
                cleared = false;
                break;
            }
        }
    ...
    // Shift all levels from the top to the lowest cleared level `cleared_lines` levels down.
    ...
    // Clear consecutive lines, shift down, rinse and repeat until the block has no cleared levels.
