...
void Tetris::spawn_block(const Vector2I &p_pos) {
  ...
  switch (dist(gen)) {
  case 1:
    next_block = std::make_unique<OTetroid>(p_pos, Renderer::GREEN);
    break;
  case 2:
  ...
  for (int y = 0; y <= Level::LVL_SIZE_Y; y++) {
    for (int x = 0; x <= Level::LVL_SIZE_X; x++) {
      if (x == 0 || y == Level::LVL_SIZE_Y || x == Level::LVL_SIZE_X) {
        ...
        Level::get_singleton().set_cell({x, y}, {Renderer::TRANSPARENT});
      }
    }
  }
}
...
void Tetris::_update(double p_delta) {
  ...
  if (ticks > std::max(1.0, 10.0 - score / 10.0)) {
    ticks = 0;

    if (curr_block->move({0, 1}) == -1) {
      // If spawn has a block in it, you lose.
      if (Level::get_singleton().get_cell(C_SPAWN_POS).color !=
...
void Tetris::_draw() {
      ...
      for (int x = 0; x <= Level::LVL_SIZE_X; x++) {
        ...
        for (int i = 0; i < CELL_SIZE * CELL_SIZE; i++) {
          m_renderer.draw_pixel(
              {x * CELL_SIZE + i % CELL_SIZE, y * CELL_SIZE + i / CELL_SIZE},
              (cell.color == Renderer::TRANSPARENT ? Level::bg : cell.color));
        }
      }
    }
  }
  ...
  //   for (int y = 0; y < Renderer::RESOLUTION.y; y++) {
  //     for (int x = UI_START; x < Renderer::RESOLUTION.x; x++) {
  //       m_renderer.draw_pixel({x, y}, Renderer::CYAN);
  //     }
  //   }
  //
  //   m_renderer.print_term(" TERTIS ", {UI_START + 1, 2}, Renderer::BLACK,
  //                         Renderer::WHITE);
  //   m_renderer.print_term(" Terminal  ", {UI_START, 4}, Renderer::BLACK,
  //                         Renderer::WHITE);
  //   m_renderer.print_term("    Tetris ", {UI_START, 5}, Renderer::BLACK,
  //                         Renderer::WHITE);
  //   m_renderer.print_term(" by", {UI_START, 5}, Renderer::BLACK,
  //                         Renderer::WHITE);
  //   m_renderer.print_term(" PhairZ ", {UI_START + 3, 6}, Renderer::BLUE,
  //                         Renderer::WHITE);
  //
  //   char buf[6];
  //   snprintf(buf, 6, "%5d",
  //            m_singles + m_doubles * 2 + m_triples * 3 + m_quadruples * 4);
  //   m_renderer.print_term("Lines:", {UI_START, 10}, Renderer::WHITE,
  //                         Renderer::CYAN);
  //   m_renderer.print_term(buf, {UI_START + 6, 10}, Renderer::BLACK,
  //                         Renderer::WHITE);
  //
  //   snprintf(buf, 6, "%5d",
  //            m_singles * 10 + m_doubles * 30 + m_triples * 50 +
  //                m_quadruples * 70);
  //   m_renderer.print_term("Score:", {UI_START, 12}, Renderer::WHITE,
  //                         Renderer::CYAN);
  //   m_renderer.print_term(buf, {UI_START + 6, 12}, Renderer::BLACK,
  //                         Renderer::WHITE);
  //
  //   // Next Block. (4x4)
  //   for (int y = 15; y < 19; y++) {
  ...
  //       auto tile = std::find(shape.begin(), shape.begin() + 4,
  //                             Vector2I(x - 15, y - 16));
