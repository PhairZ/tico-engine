...
void Block::clear() {
  ...
  for (size_t i = 0; i < 4; i++) {
    auto &tile = shape[i + m_rot_offset];
    lvl.set_cell(m_position + tile, Renderer::TRANSPARENT);
  }
}
...
int Block::move(const Vector2I &p_offset) {
  ...
  for (size_t i = 0; i < 4; i++) {
    auto &tile = shape[i + m_rot_offset];
    bool is_self = std::any_of(shape.begin() + m_rot_offset,
                               shape.begin() + m_rot_offset + 4,
                               [p_offset, tile](const Vector2I &vec) {
                                 return vec == (tile + p_offset);
                               });

    if (!is_self) {
      bool collided = lvl.get_cell(m_position + p_offset + tile).color !=
...
int Block::rotate(bool p_ccw) {
  ...
  for (size_t i = 0; i < 4; i++) {
    ...
    if (!is_self) {
      bool collided =
          lvl.get_cell(m_position + new_shape).color != Renderer::TRANSPARENT;
      if (collided)
        return -1;
      ;
    }
