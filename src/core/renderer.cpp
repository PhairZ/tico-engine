...
void Renderer::draw_pixel(const Vector2I &p_pos, const Color p_color) {
  if (p_pos.x < 0 || p_pos.x >= RESOLUTION.x || p_pos.y < 0 ||
      p_pos.y >= RESOLUTION.y) {
    return;
  }
...
void Renderer::draw_rect(const Vector2I &p_position, const Vector2I &p_size,
...
void Renderer::draw_sprite(const char *p_sprite, const Vector2I &p_position,
                           bool flip_h, bool flip_v, const Vector2I &p_size) {
  for (int y = 0; y < p_size.y; y++) {
    for (int x = 0; x < p_size.x; x++) {
      ...
      Vector2I pos = p_position + Vector2I(flip_h ? p_size.x - x - 1 : x,
                                           flip_v ? p_size.y - y - 1 : y);
...
void Renderer::print_term(const Printable &p_text, const Vector2I &p_pos,
                          const Color p_fg_color, const Color p_bg_color) {
...
void Renderer::print(const Printable &p_text, const Vector2I &p_pos,
                     const Color p_fg_color, const Color p_bg_color) {
...
void Renderer::render_screen() {
  ...
  WriteConsoleW(GetStdHandle(STD_OUTPUT_HANDLE), render.c_str(), render.size(),
                nullptr, nullptr);
#else
