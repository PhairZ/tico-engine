...
void SMB::_init() {
	m_renderer.set_resolution({ 256, 160 });
}
	...
	if (InputEventKey* key_event = dynamic_cast<InputEventKey*>(p_event)) {
		if (key_event->pressed) {
			...
			case 'd':
				...
				break;
			case 'A':
				m_mario.velocity.x = std::lerp(m_mario.velocity.x, -Player::SPEED * 2.0, 0.03);
				m_mario.flip_h = true;
				break;
			case 'D':
				m_mario.velocity.x = std::lerp(m_mario.velocity.x, Player::SPEED * 2.0, 0.03);
			...
			case ' ':
				if (m_mario.is_on_floor()) {
					m_mario.velocity.y = Player::JUMP_FORCE - std::abs(m_mario.velocity.x * 4);
				}
				break;
			case 'r':
...
void SMB::_update(double p_delta) {
	...
	if (m_mario.position.x - m_cam_pos.x > 96) {
		m_cam_pos.x = m_mario.position.x - 96;
	...
	// Limit fall speed.
	if (abs(m_mario.velocity.y) > m_mario.TERMINAL_VELOCITY) {
		m_mario.velocity.y = m_mario.TERMINAL_VELOCITY * (m_mario.velocity.y > 0 ? 1 : -1);
	}
}
...
void SMB::draw_map(const Vector2I& p_offset) {
	for (int y = 0; y < Sprites::C_MAP_SIZE.y; y++) {
		for (int x = 0; x < Sprites::C_MAP_SIZE.x; x++) {
			...
			if (sprite_index > 0) {
				m_renderer.draw_sprite(Sprites::C_SPRITE_MAP[sprite_index], p_offset + Vector2I(x * 16, y * 16));
			}
		}
	}
}
	...
	for (int y = 0; y < Sprites::C_BG_SIZE.y; y++) {
		for (int x = 0; x < Sprites::C_BG_SIZE.x; x++) {
			...
			if (sprite_index > 0 && sprite_index < 99) {
				m_renderer.draw_sprite(Sprites::C_BG_SPRITE_MAP[sprite_index], p_offset + Vector2I(x * 16, y * 16));
			}
		}
	}
}
...
void SMB::_draw() {
	for (int y = 0; y < m_renderer.get_resolution().y; y++) {
		for (int x = 0; x < m_renderer.get_resolution().x; x++) {
	...
	m_renderer.print("FPS:" + std::to_string(1.0 / m_delta_time), { 0, 0 }, Renderer::WHITE, Renderer::BLACK);
	//m_renderer.print("Vel:" + m_mario.velocity.to_string(), { 0, 7 }, Renderer::WHITE);
	//m_renderer.print("Pos:" + m_mario.position.to_string(), { 0, 13 }, Renderer::WHITE);
	//m_renderer.print("On Floor:" + std::string(m_mario.is_on_floor() ? "True" : "False"), {0, 20}, Renderer::WHITE);
}